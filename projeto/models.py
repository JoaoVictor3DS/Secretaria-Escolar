...
from .validators import validate_cpf, validate_nota, validate_phone
import re
            ...
            for materia in materias:
                if materia:
                    Notas.objects.get_or_create(
                        aluno=self, turma=turma, materia=materia
                    )
...
class Professor(models.Model):
    ...
    def __str__(self):
        return (
            f"{self.first_name} {self.last_name} (Registro: {self.registration_number})"
        )
...
class Turma(models.Model):
    ...
    vice_representante = models.ForeignKey(
        Aluno,
        on_delete=models.SET_NULL,
...
class Contrato(models.Model):
    ...
    aluno = models.ForeignKey(
        Aluno, on_delete=models.CASCADE, verbose_name="Nome do Aluno"
    )
    responsavel = models.ForeignKey(
        Responsavel,
        on_delete=models.CASCADE,
    ...
    email_responsavel = models.EmailField(
        verbose_name="Email do Responsável", blank=True, null=True
    )
        ...
        if self.aluno and self.responsavel:
            if self.aluno not in self.responsavel.alunos.all():
                raise ValidationError(
                    "O aluno selecionado não pertence ao responsável informado."
                )
        super().save(*args, **kwargs)
...
class Nota(models.Model):
    ...
    materia = models.ForeignKey(
        Materia, on_delete=models.CASCADE, verbose_name="Matéria"
    )
    aluno = models.ForeignKey(Aluno, on_delete=models.CASCADE, verbose_name="Aluno")
    nota_presenca = models.FloatField(
        verbose_name="Nota de Presença (Diário)", default=0, validators=[validate_nota]
    )
    nota_atividade = models.FloatField(
        verbose_name="Nota de Atividade (Diário)", default=0, validators=[validate_nota]
    )
    nota_avaliativa = models.FloatField(
        verbose_name="Nota Avaliativa (Mensal)", default=0, validators=[validate_nota]
    )
    ...
    class Meta:
        unique_together = (
            ...
            "materia",
            "aluno",
        )  # Ensure unique Notas per aluno, turma, and materia

    def save(self, *args, **kwargs):
        self.nota_final = (
            self.nota_presenca + self.nota_atividade + self.nota_avaliativa
        ) / 3
        super().save(*args, **kwargs)
...
class DesempenhoAcademico(models.Model):
    ...
    def save(self, *args, **kwargs):
        self.media_final = (
            self.bimestre_1 + self.bimestre_2 + self.bimestre_3 + self.bimestre_4
        ) / 4
        super().save(*args, **kwargs)
...
class Presenca(models.Model):
    ...
    status = models.CharField(
        max_length=1, choices=[("F", "Falta"), ("P", "Presente")], verbose_name="Status"
    )
...
class Agenda(models.Model):
    ...
    arquivo = models.FileField(
        upload_to="agenda_files/", blank=True, null=True, verbose_name="Arquivo"
    )
